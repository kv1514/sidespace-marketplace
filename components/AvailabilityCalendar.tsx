...
export function AvailabilityCalendar({ selected, onChange, minimum, maximum, allowed, multiple = false }: {
  ...
  return <div className="availability-calendar" aria-labelledby={heading}>
    <div className="availability-calendar-nav">
      <button type="button" aria-label="Previous month" disabled={previous < minimum.slice(0, 7)} onClick={() => setMonth(previous)}>‹</button>
      <strong id={heading} aria-live="polite">{title}</strong>
      <button type="button" aria-label="Next month" disabled={next > maximum.slice(0, 7)} onClick={() => setMonth(next)}>›</button>
    </div>
    ...
    {multiple && <div className="availability-calendar-shortcuts">
      <button type="button" onClick={() => onChange([...new Set([...selected, ...monthDates])].sort())}>Select month</button>
      <button type="button" onClick={() => onChange([...new Set([...selected, ...monthDates.filter((date) => ![0, 6].includes(new Date(`${date}T12:00:00Z`).getUTCDay()))])].sort())}>Weekdays</button>
      <button type="button" onClick={() => onChange(selected.filter((date) => !date.startsWith(month)))}>Clear month</button>
    </div>}
  </div>;
}
...
export function ListingAvailabilityFields({ listing = {}, onChange }: {
  ...
  return <section className="listing-availability field-wide">
    <label className="instant-booking-toggle">
      <span><strong>Let businesses book instantly</strong><small>Set your dates once. Buyers choose a date and check out.</small></span>
    ...
    {enabled && <div className="listing-availability-body">
      <label>Days included in one package
        <small>Your listed price covers this entire booking. Only one business can book these dates.</small>
      ...
      <p>Select every day you can deliver, up to one year ahead. For a longer package, select consecutive days.</p>
      ...
      <div className="calendar-legend"><span><i /> Available</span><span>{dates.length} days selected · {timeZone.replaceAll("_", " ")}</span></div>
      <p className="instant-booking-commitment">By enabling this, you agree to fulfill paid bookings on these dates at your fixed price, with the deliverables and cancellation terms below. Finish payout setup before buyers can pay. Calendar changes affect new bookings; existing bookings stay reserved.</p>
    </div>}
  </section>;
}
