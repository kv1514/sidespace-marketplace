...
export function bookingDateLabel(kind: TimingKind | null | undefined, start: string, end: string, t: Translate = english) {
  const tag = LOCALE_TAGS[t.locale];
  ...
  if (!validCalendarDay(start) || !validCalendarDay(end)) return t("Choose a date");
  if (kind === "deadline") return t("Deliver by {date}", { date: label(end) });
  ...
  return days === 1 ? t("{range} · 1 day", { range }) : t("{range} · {count} days", { range, count: days });
}
