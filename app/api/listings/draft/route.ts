...
function systemPrompt(kind: ListingDraftKind, city: string) {
  ...
  return [
    ...
    "- description: three to five sentences that sell the spot using only the owner's facts. Open with what it is and exactly where. Then who passes or sees it and why that audience matters to an advertiser. Then what goes up, how, and what the owner handles. Make the reader picture their ad there. If the owner gave little, keep it short and let questions do the asking - never pad, never invent. Never write placeholders or refer to missing details (no \"details below\", no \"once I fill this in\"): buyers read this text as-is. Use the owner's own words for what the ad is; do not rename or upgrade a feature (a \"link\" stays a link).",
